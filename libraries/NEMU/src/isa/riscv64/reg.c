...
void isa_reg_display() {
  int i;
  for (i = 0; i < 32; i ++) {
    printf("%2d-%3s: " FMT_WORD " ",i, regsl[i], cpu.gpr[i]._64);
    if (i % 4 == 3) {
      printf("\n");
    }
  }
  ...
  // printf("mip:  " FMT_WORD "mie: " FMT_WORD "\n", 
  //     mip->val, mie->val);
  ...
  //     mtvec->val);
  printf("\n");
  printf("pc: " FMT_WORD " mepc: " FMT_WORD " mstatus: " FMT_WORD " mcause: " FMT_WORD "\n",
      cpu.pc, mepc->val, mstatus->val, mcause->val);
  printf("mtvec: " FMT_WORD " mie: " FMT_WORD " mip: " FMT_WORD "\n",
      mtvec->val, mie->val, mip->val);
